...
// Gemini CLI prefers GOOGLE_API_KEY but also accepts GEMINI_API_KEY
...
try {
  ...
  console.log(`[Setup] GOOGLE_API_KEY present: ${!!GOOGLE_API_KEY}`);
  console.log(`[Setup] CLAUDE_CODE_OAUTH_TOKEN present: ${!!CLAUDE_CODE_OAUTH_TOKEN}`);

  const isGemini = AI_PROVIDER === 'gemini';
  const hasCredentials = isGemini ? !!GOOGLE_API_KEY : !!CLAUDE_CODE_OAUTH_TOKEN;

  console.log(`[Setup] isGemini: ${isGemini}`);
  console.log(`[Setup] hasCredentials: ${hasCredentials}`);

  if (hasCredentials && USER_PROMPT) {
    const modelName = isGemini ? 'Gemini 3 Pro' : 'Claude Sonnet 4';
    await sendSystemMessage(`[Setup] 💬 🤖 Starting ${isGemini ? 'Gemini' : 'Claude'} agent (model: ${modelName})...`);
    ...
    // The correct script (Claude or Gemini) is uploaded based on AI_PROVIDER
    // If not, the setup script was called incorrectly

    // Check if agent script exists
    console.log(`[Setup] Checking for agent script at /home/user/stream-to-supabase.js...`);
    ...
    if (!scriptExists) {
      ...
      // Start the AI agent script (Claude or Gemini based on AI_PROVIDER)
      const agentScript = '/home/user/stream-to-supabase.js';
      const logFile = isGemini ? '/home/user/gemini-agent.log' : '/home/user/claude-agent.log';
      ...
      if (isGemini) {
        agentEnv.GOOGLE_API_KEY = GOOGLE_API_KEY;
        agentEnv.GEMINI_API_KEY = GEMINI_API_KEY;
        console.log(`[Setup] Added GOOGLE_API_KEY and GEMINI_API_KEY to agent environment`);
      } else {
      ...
      if (agentPid) {
        console.log(`[Setup] ✓ ${isGemini ? 'Gemini' : 'Claude'} agent started (PID: ${agentPid})`);
        console.log(`[Setup] ✓ Model: ${modelName}`);
        await sendSystemMessage(`[Setup] ✓ ${isGemini ? 'Gemini' : 'Claude'} agent started (PID: ${agentPid})`);
        ...
        await sendSystemMessage(`[Setup] ✗ Failed to start ${isGemini ? 'Gemini' : 'Claude'} agent`, 'error');
      }
    }
  } else {

...
try {
  ...
  // Step 3.5: Install tmux for programmatic Metro control
  await logOperation('tmux_install', 'started', '[Setup] 💬 📦 Installing tmux for Metro control...');
  console.log('[Setup] Step 3.5: Installing tmux...');
  await new Promise((resolve, reject) => {
    const tmuxInstall = spawn('sh', ['-c', 'which tmux > /dev/null 2>&1 || (apt-get update > /dev/null 2>&1 && apt-get install -y tmux > /dev/null 2>&1)'], {
      stdio: 'inherit',
    });

    tmuxInstall.on('close', async (code) => {
      if (code === 0) {
        await logOperation('tmux_install', 'completed', '[Setup] ✓ tmux installed successfully');
        resolve();
      } else {
        // tmux installation failed, but we'll try to continue anyway
        await logOperation('tmux_install', 'failed', '[Setup] ⚠️ tmux installation failed, Metro reload may not work', { error: 'Installation returned non-zero exit code' });
        console.warn('[Setup] ⚠️ tmux installation failed, Metro reload may not work');
        resolve();
      }
    });
    tmuxInstall.on('error', async (err) => {
      await logOperation('tmux_install', 'failed', '[Setup] ⚠️ tmux installation error, Metro reload may not work', { error: err.message });
      console.warn('[Setup] ⚠️ tmux installation error, Metro reload may not work');
      resolve();
    });
  });
  ...
  const expoProcess = spawn('tmux', [
    ...
    '-s',
    'metro',
    `cd ${PROJECT_DIR} && NODE_OPTIONS="--max-old-space-size=3072" npx expo start --tunnel 2>&1 | tee /home/user/expo.log`
  ], {
    stdio: 'ignore',
    detached: true,
  });
  ...
  await logOperation('metro_start', 'progress', '[Setup] ⏳ Metro process spawned, waiting for bundler to be ready...');

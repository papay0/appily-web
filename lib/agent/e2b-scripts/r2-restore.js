...
async function restoreFromR2(options) {
  ...
  try {
    ...
    // Files to skip (core dumps, etc.)
    const skipFiles = ['core'];

    // Maximum file size to restore (50MB)
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (skipFiles.includes(filename)) {
        console.log(`[r2-restore] Skipping excluded file: ${relativePath}`);
        skippedCount++;
        continue;
      }
      ...
      if (file.Size && file.Size > MAX_FILE_SIZE) {
        console.log(`[r2-restore] Skipping large file (${(file.Size / 1024 / 1024).toFixed(2)} MB): ${relativePath}`);
        skippedCount++;
        continue;
      }
      ...
      try {
        ...
        if (body.length > MAX_FILE_SIZE) {
          console.log(`[r2-restore] Skipping downloaded large file (${(body.length / 1024 / 1024).toFixed(2)} MB): ${relativePath}`);
          skippedCount++;
          continue;
        }
        ...
        console.warn(`[r2-restore] Failed to download ${relativePath}:`, downloadError.message);
        skippedCount++;
        continue;
      }
    ...
    if (skippedCount > 0) {
      console.log(`[r2-restore] Skipped ${skippedCount} files (too large or excluded)`);
    }

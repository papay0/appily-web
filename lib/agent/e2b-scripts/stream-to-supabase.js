    ...
    if (capturedSessionId) {
      ...
      if (error) {
        ...
        if (event.type === 'result') {
          ...
          // Trigger save to R2 after successful task completion
...
/**
 * Trigger save to R2 via internal API endpoint
 */
async function triggerSaveToR2() {
  try {
    console.log('[E2B] 💾 Triggering auto-save to R2...');

    // Use global fetch (Node.js 18+ has built-in fetch support)
    const response = await fetch(`${API_URL}/api/projects/${PROJECT_ID}/save-internal`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      console.log(`[E2B] ✓ Project saved to R2: v${data.snapshot?.version}, ${data.snapshot?.fileCount} files`);
    } else {
      const error = await response.json();
      console.error('[E2B] ✗ Failed to save to R2:', error.error || response.statusText);
    }
  } catch (error) {
    console.error('[E2B] ✗ Error triggering save to R2:', error.message);

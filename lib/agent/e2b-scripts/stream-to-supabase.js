...
async function triggerSaveToR2() {
  try {
    ...
    if (result.success) {
      console.log(`[E2B] ✓ Project saved to R2: v${result.version}, ${result.fileCount} files`);
    } else {
      console.error('[E2B] ✗ Failed to save to R2:', result.error);
    }
  } catch (error) {
...
async function triggerMetroReload() {
  try {
    console.log('[E2B] 🔄 Triggering Metro reload...');
    ...
    if (result.success) {
      console.log('[E2B] ✓ Metro reloaded - changes should appear in Expo Go');
    } else {
      console.log('[E2B] ⚠️ Metro reload skipped:', result.error);
    }
  } catch (error) {
    console.error('[E2B] ✗ Error triggering Metro reload:', error.message);

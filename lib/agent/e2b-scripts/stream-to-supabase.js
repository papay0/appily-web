...
const args = SESSION_ID
  ? ['-r', SESSION_ID, '-p', USER_PROMPT, '--output-format', 'stream-json', '--verbose', '--dangerously-skip-permissions']
  : ['-p', USER_PROMPT, '--output-format', 'stream-json', '--verbose', '--dangerously-skip-permissions'];
    ...
    if (event.type === 'system' && event.subtype === 'init' && event.session_id) {
      ...
      const { error: sessionError } = await supabase
        ...
        .insert({
          ...
          user_id: process.env.USER_ID, // Optional, may be undefined
      ...
      if (sessionError) {
        console.error('[E2B] Failed to store session:', sessionError.message);
      } else {
        console.log('[E2B] ✓ Session stored in database');
      }
    }
...
claude.stdout.on('data', (data) => {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      // Not valid JSON, skip (could be non-JSON output)
    }
  }
});
...
claude.stderr.on('data', (data) => {
  ...
  for (const line of lines) {
    ...
    if (line.includes('error') || line.includes('Error') || line.includes('failed')) {
      console.error(`[E2B] ⚠️ ${line.substring(0, 200)}`);
    }
  }
});
...
claude.on('close', async (code) => {
  console.log(`[E2B] Claude process exited with code ${code}`);
  console.log(`[E2B] Total events processed: ${eventCount}`);
  ...
  if (capturedSessionId) {
    try {
      ...
      if (error) {
        console.error('[E2B] Failed to update session status:', error.message);
      } else {
        console.log(`[E2B] ✓ Session marked as ${code === 0 ? 'completed' : 'error'}`);
      }
    } catch (error) {
      console.error('[E2B] Error updating session:', error.message);
    }
  }

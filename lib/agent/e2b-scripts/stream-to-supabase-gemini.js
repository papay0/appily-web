...
if (!GOOGLE_API_KEY) {
  console.error('[E2B] ERROR: Missing GOOGLE_API_KEY');
  process.exit(1);
}
...
await sendDebugMessage(`[Gemini] GOOGLE_API_KEY present: ${!!GOOGLE_API_KEY}`);
console.log(`[E2B] GOOGLE_API_KEY length: ${GOOGLE_API_KEY ? GOOGLE_API_KEY.length : 0}`);
...
const gemini = spawn('gemini', args, {
  ...
  env: {
    ...process.env,
    GOOGLE_API_KEY,
    GEMINI_API_KEY: GOOGLE_API_KEY, // Pass both variants
...
await sendDebugMessage('[Gemini] Writing prompt to stdin...');
gemini.stdin.write(USER_PROMPT);
gemini.stdin.end();
await sendDebugMessage('[Gemini] ✅ Prompt sent via stdin');

await sendDebugMessage(`[Gemini] ✅ Process spawned with PID: ${gemini.pid}`);
...
gemini.stdout.on('data', async (data) => {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      await sendDebugMessage(`[Gemini] Event: ${event.type}${event.type === 'message' ? ` (${event.role})` : ''}`);
      enqueueStore(event);
    } catch {

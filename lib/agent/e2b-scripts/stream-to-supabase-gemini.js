...
async function flushAssistantBuffer() {
  if (!assistantMessageBuffer) {
    console.log('[E2B] flushAssistantBuffer: buffer empty');
    return;
  }

  const bufferText = assistantMessageBuffer;
  const bufferTimestamp = assistantMessageTimestamp;
  ...
  console.log(`[E2B] FLUSHING ASSISTANT: ${bufferText.length} chars`);
  console.log(`[E2B] Event structure: ${JSON.stringify(event).substring(0, 200)}...`);

  // Store with .select() to verify success
  const { error, data } = await supabase.from('agent_events').insert({
    session_id: capturedSessionId || null,
    project_id: PROJECT_ID,
    event_type: 'assistant',
    event_data: event,
    created_at: bufferTimestamp || new Date().toISOString(),
  }).select('id');

  if (error) {
    console.error('[E2B] ❌ ASSISTANT INSERT FAILED:', error.message);
    await sendDebugMessage(`[Gemini] ❌ Failed to save assistant message: ${error.message}`, 'error');
  } else {
    eventCount++;
    console.log(`[E2B] ✅ ASSISTANT INSERT SUCCESS: id=${data?.[0]?.id}`);
    const preview = bufferText.substring(0, 100);
    await sendDebugMessage(`[Gemini] ✅ Assistant message saved (${bufferText.length} chars)`);
  }
}
...
gemini.stdout.on('data', async (data) => {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      // Log FULL event JSON for debugging (NO TRUNCATION)
      const eventJson = JSON.stringify(event);
      console.log(`[E2B] FULL EVENT: ${eventJson}`);
      await sendDebugMessage(`[Gemini] 📦 ${event.type}: ${eventJson}`);
...
gemini.on('close', async (code) => {
  ...
  // Ensure assistant buffer is flushed before exit
  console.log(`[E2B] Pre-exit: assistant buffer has ${assistantMessageBuffer.length} chars`);
  await flushAssistantBuffer();
